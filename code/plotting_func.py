...
from quality_metrics_func import batch_psnr_numpy
...
def plot_denoised_range(clean, noisy, denoised, noise_range, file_name,data_max):
...
def plot_many_denoised(x, y, x_hat,  device, suptitle, label, train, vmin=0, vmax=1, im_size=3, n_columns=7): 
    ...
    if device.type == 'cuda':
        for i in range(len(y)): 
            y[im_labels[i]] = y[im_labels[i]].cpu().squeeze().numpy()
    ...
    if device.type == 'cuda':
        for i in range(len(x_hat)): 
            x_hat[im_labels[i]] = x_hat[im_labels[i]].cpu().squeeze().numpy()
...
def show_im_set(dataset , N=None , im_size=3,vmin=None, vmax=None, label='None', colorbar=False,n_columns=7, sub_labels=None):
    if N is None:
        N = dataset.shape[0]
     
    if dataset.device.type == 'cuda':
        dataset = dataset.cpu().squeeze(1)
    else: 
    ...
    for i in range(N ): 
        fig = axs[i].imshow(dataset[i], 'gray',vmin=vmin, vmax = vmax)
        if colorbar:
            plt.colorbar(fig, ax=axs[i], fraction=.05)
        if sub_labels is not None: 
            axs[i].set_title( str(sub_labels[i]))
